...
namespace Bedrock.Infrastructure.Repositories
{
    ...
    public class AppRepository : IAppRepository
    {
        ...
        public AppRepository(ISqlSugarClient db)
        {
            _db = db;
        }
        ...
        public async Task<long> CreateAsync(App entity)
        {
        ...
        public async Task<int> UpdateAsync(App entity)
        {
        ...
        /// <summary>
        /// 软删除指定的App应用记录（标记 DeletedAt 和 DeletedById）。
        /// </summary>
        /// <param name="entity">要删除的实体，用于获取 Id 和审计信息。</param>
        /// <returns>返回受影响的记录数（1 或 0）。</returns>
        public async Task<int> DeleteAsync(App entity)
        {
            var rowsAffected = await _db.Updateable(entity).ExecuteCommandAsync();
            return rowsAffected;
        }
        ...
        /// <param name="operatorId">执行删除操作的App应用 ID，用于审计。</param>
        ...
        public async Task<int> DeleteBatchAsync(IEnumerable<long> ids, long operatorId)
        {
            ...
            var rowsAffected = await _db.Updateable<App>()
                .SetColumns(e => new App
                {
        ...
        {
            var entity = await _db.Queryable<App>()
        ...
        /// <summary>
        /// 根据 ID 列表批量获取未删除的App应用实体。
        /// </summary>
        /// <param name="ids">App应用 ID 列表。</param>
        /// <returns>返回匹配的实体列表（可能为空）。</returns>
        public async Task<List<App>> GetByIdsAsync(IEnumerable<long> ids)
        {
            if (ids == null)
                return new List<App>();
            ...
            return await _db.Queryable<App>()
        ...
        /// <summary>
        /// 查询未删除的App应用记录，支持按App应用名称模糊搜索。
        /// </summary>
        /// <param name="appId">App应用名称，用于模糊搜索（可选）。</param>
        /// <returns>匹配条件的未删除记录集合（可能为空）。</returns>
        public async Task<List<App>> GetAllAsync(string? name = null)
        {
            var query = _db.Queryable<App>().Where(e => e.DeletedAt == null);

            if (!string.IsNullOrWhiteSpace(name))
            {
                query = query.Where(e => e.AppId.Contains(name));
            }
        ...
        /// </returns>
        public async Task<(List<App> Data, int TotalCount)> GetPagedAsync(
            int pageNumber,
            int pageSize,
            string? name = null,
        ...
        /// <returns>匹配的实体；若未找到或已删除，则返回 null。由于 AppId 唯一，最多返回一条。</returns>
        public async Task<App?> GetByAppIdAsync(string appId)
        {
