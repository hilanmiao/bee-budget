using Bedrock.Application.Interfaces;
using Bedrock.Core.Entities;
using SqlSugar;
using System;

namespace Bedrock.Infrastructure.Repositories
{
    /// <summary>
    /// 用户角色关联仓储实现类，用于管理用户角色关联的增删改查操作。
    /// </summary>
    public class SysUserRoleRepository : ISysUserRoleRepository
    {
        private readonly ISqlSugarClient _db; // 数据库上下文实例

        /// <summary>
        /// 构造函数注入数据库上下文。
        /// </summary>
        /// <param name="db">SqlSugar 数据库上下文。</param>
        public SysUserRoleRepository(ISqlSugarClient db)
        {
            _db = db;
        }

        /// <summary>
        /// 创建用户角色关联。
        /// </summary>
        /// <param name="entity">要创建的用户角色关联实体对象。</param>
        /// <returns>返回受影响的记录数（通常为 1，若记录不存在则为 0）。</returns>
        public async Task<int> CreateAsync(SysUserRole entity)
        {
            var rowsAffected = await _db.Insertable(entity).ExecuteCommandAsync();
            return rowsAffected;
        }

        /// <summary>
        /// 批量创建用户角色关联。
        /// </summary>
        /// <param name="entities">要创建的用户角色关联实体对象集合。</param>
        /// <returns>返回成功插入的记录数量；若集合为空，则返回 0。</returns>
        public async Task<int> CreateBatchAsync(IEnumerable<SysUserRole> entities)
        {
            var entityArray = entities as SysUserRole[] ?? entities.ToArray();
            if (!entityArray.Any())
                return 0;

            var rowsAffected = await _db.Insertable(entities.ToList()).ExecuteCommandAsync();
            return rowsAffected;
        }

        /// <summary>
        /// 删除用户角色关联。
        /// </summary>
        /// <param name="entity">要删除的用户角色关联实体对象。</param>
        /// <returns>返回成功标记为删除的记录数量。</returns>
        public async Task<int> DeleteAsync(SysUserRole entity)
        {
            var rowsAffected = await _db.Deleteable<SysUserRole>()
                .Where(ur => ur.UserId == entity.UserId && ur.RoleId == entity.RoleId)
                .ExecuteCommandAsync();
            return rowsAffected;
        }

        /// <summary>
        /// 根据角色ID删除所有关联的用户。
        /// </summary>
        /// <param name="roleId">角色ID。</param>
        /// <returns>返回成功标记为删除的记录数量。</returns>
        public async Task<int> DeleteByRoleIdAsync(long roleId)
        {
            var rowsAffected = await _db.Deleteable<SysUserRole>()
                .Where(ur => ur.RoleId == roleId)
                .ExecuteCommandAsync();
            return rowsAffected;
        }

        /// <summary>
        /// 根据用户ID删除所有关联的角色。
        /// </summary>
        /// <param name="userId">用户ID。</param>
        /// <returns>返回成功标记为删除的记录数量。</returns>
        public async Task<int> DeleteByUserIdAsync(long userId)
        {
            var rowsAffected = await _db.Deleteable<SysUserRole>()
                .Where(ur => ur.UserId == userId)
                .ExecuteCommandAsync();
            return rowsAffected;
        }

        /// <summary>
        /// 根据角色ID获取所有关联的用户ID。
        /// </summary>
        /// <param name="roleId">角色ID。</param>
        /// <returns>返回与角色关联的用户ID集合。</returns>
        public async Task<List<long>> GetUserIdsByRoleIdAsync(long roleId)
        {
            var UserIds = await _db.Queryable<SysUserRole>()
                .Where(ur => ur.RoleId == roleId)
                .Select(ur => ur.UserId)
                .ToListAsync();

            return UserIds;
        }

        /// <summary>
        /// 根据用户ID获取所有关联的角色ID。
        /// </summary>
        /// <param name="userId">用户ID。</param>
        /// <returns>返回与用户关联的角色ID集合。</returns>
        public async Task<List<long>> GetRoleIdsByUserIdAsync(long userId)
        {
            var roleIds = await _db.Queryable<SysUserRole>()
                .Where(ur => ur.UserId == userId)
                .Select(ur => ur.RoleId)
                .ToListAsync();

            return roleIds;
        }

        /// <summary>
        /// 检查角色和用户是否已关联。
        /// </summary>
        /// <param name="roleId">角色ID。</param>
        /// <param name="userId">用户ID。</param>
        /// <returns>如果已关联返回true，否则返回false。</returns>
        public async Task<bool> ExistsAsync(long roleId, long userId)
        {
            var exists = await _db.Queryable<SysUserRole>()
                .Where(ur => ur.RoleId == roleId && ur.UserId == userId)
                .AnyAsync();

            return exists;
        }

        /// <summary>
        /// 获取所有用户角色关联。
        /// </summary>
        /// <returns>返回所有用户角色关联的集合。</returns>
        public async Task<List<SysUserRole>> GetAllAsync()
        {
            var entities = await _db.Queryable<SysUserRole>()
                .ToListAsync();

            return entities;
        }

        /// <summary>
        /// 根据用户ID列表，批量软删除用户记录，并记录操作人。
        /// </summary>
        /// <param name="userIds">要删除的用户 ID 列表。</param>
        /// <returns>返回成功标记为删除的记录数量；若用户 ID 列表为空，则返回 0。</returns>        
        public async Task<int> DeleteByUserIdsAsync(IEnumerable<long> userIds)
        {
            var userIdList = userIds as long[] ?? userIds.ToArray();
            if (!userIdList.Any())
                return 0;

            var rowsAffected = await _db.Deleteable<SysUserRole>()
                .Where(e => userIdList.Contains(e.UserId))
                .ExecuteCommandAsync();

            return rowsAffected;
        }
    }
}