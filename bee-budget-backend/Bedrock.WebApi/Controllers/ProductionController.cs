...
namespace Bedrock.WebApi.Controllers
{
    ...
    public class ProductionController : ControllerBase
    {
        ...
        /// 构造函数注入服务层依赖。
        /// </summary>
        /// <param name="productionService">产品业务逻辑服务。</param>
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] CreateProductionDto createDto)
        {
            if (createDto == null)
            {
            ...
            var id = await _productionService.CreateAsync(createDto, currentUser.Id);
            var dto = await _productionService.GetAsync(id);
            var responseCreate = ApiResponse<ProductionDto>.OperationSuccess(dto);
        ...
        [HttpPost("batch")]
        public async Task<IActionResult> CreateBatchAsync([FromBody] List<CreateProductionDto> createDtos)
        {
            ...
            var createdCount = await _productionService.CreateBatchAsync(createDtos, currentUser.Id);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(long id, [FromBody] UpdateProductionDto updateDto)
        {
            if (updateDto == null)
            {
                var response = ApiResponse<object>.BadRequest("请求数据不能为空。");
            ...
            var updatedId = await _productionService.UpdateAsync(updateDto, currentUser.Id);
        ...
        [HttpPut("batch")]
        public async Task<IActionResult> UpdateBatchAsync([FromBody] List<UpdateProductionDto> updateDtos)
        {
            ...
            var updatedCount = await _productionService.UpdateBatchAsync(updateDtos, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteAsync(long id)
        {
            ...
            var deletedId = await _productionService.DeleteAsync(id, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteBatchAsync([FromBody] DeleteBatchRequest batchRequestDto)
        {
            ...
            var deletedCount = await _productionService.DeleteBatchAsync(batchRequestDto.Ids, currentUser.Id);
        ...
        public async Task<IActionResult> GetAsync(long id)
        {
            ...
            if (dto == null)
            {
                var response = ApiResponse<ProductionDto>.NotFound();
                return NotFound(response);
            }

            var responseGet = ApiResponse<ProductionDto>.OperationSuccess(dto);
        ...
        {
            var dtos = await _productionService.GetAllAsync(name, status);
            var responseAll = ApiResponse<List<ProductionDto>>.OperationSuccess(dtos);
        ...
        {
            var pagedResult = await _productionService.GetPagedAsync(pageNumber, pageSize, name, model, status, orderByField, orderByType);
            var responsePaged = ApiResponse<PaginationResult<ProductionDto>>.OperationSuccess(pagedResult);
        ...
        public async Task<IActionResult> ChangeStatus(long id, [FromBody] ChangeStatusDto changeStatusDto)
        {
            ...
            var updatedId = await _productionService.ChangeStatusAsync(id, changeStatusDto.Status, currentUser.Id);
            var responseUpdate = ApiResponse<long>.OperationSuccess(updatedId);
            return Ok(responseUpdate);
        }
