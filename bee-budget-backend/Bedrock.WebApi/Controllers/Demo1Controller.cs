...
namespace Bedrock.WebApi.Controllers
{
    ...
    {
        private readonly IDemo1Service _demo1Service;

        /// <summary>
        /// 构造函数注入服务层依赖。
        /// </summary>
        /// <param name="demo1Service">样例1业务逻辑服务。</param>
        public Demo1Controller(IDemo1Service demo1Service)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] CreateDemo1Dto createDto)
        {
            if (createDto == null)
            {
            ...
            var id = await _demo1Service.CreateAsync(createDto, currentUser.Id);
            var dto = await _demo1Service.GetAsync(id);
            var responseCreate = ApiResponse<Demo1Dto>.OperationSuccess(dto);
        ...
        /// 更新指定的样例1。
        /// </summary>
        /// <param name="id">样例1 ID。</param>
        /// <param name="updateDto">更新 DTO，必须包含有效 ID。</param>
        /// <returns>返回被更新的样例1 ID。</returns>
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(long id, [FromBody] UpdateDemo1Dto updateDto)
        {
            if (updateDto == null)
            {
                var response = ApiResponse<object>.BadRequest("请求数据不能为空。");
            ...
            var updatedId = await _demo1Service.UpdateAsync(updateDto, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteAsync(long id)
        {
            ...
            var deletedId = await _demo1Service.DeleteAsync(id, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteBatchAsync([FromBody] DeleteBatchRequest request)
        {
            ...
            var deletedCount = await _demo1Service.DeleteBatchAsync(request.Ids, currentUser.Id);
        ...
        public async Task<IActionResult> ChangeStatus(long id, [FromBody] ChangeStatusDto changeStatusDto)
        {
            ...
            var updatedId = await _demo1Service.ChangeStatusAsync(id, changeStatusDto.Status, currentUser.Id);
            var responseUpdate = ApiResponse<long>.OperationSuccess(updatedId);
            return Ok(responseUpdate);
        }
        ...
        public async Task<IActionResult> GetAsync(long id)
        {
            ...
            if (dto == null)
            {
                var response = ApiResponse<Demo1Dto>.NotFound();
                return NotFound(response);
            }

            var responseGet = ApiResponse<Demo1Dto>.OperationSuccess(dto);
        ...
        {
            var dtos = await _demo1Service.GetAllAsync(name, status);
            var responseAll = ApiResponse<List<Demo1Dto>>.OperationSuccess(dtos);
        ...
        {
            var pagedResult = await _demo1Service.GetPagedAsync(pageNumber, pageSize, name, status, startDate, endDate, orderByField, orderByType);
            var responsePaged = ApiResponse<PaginationResult<Demo1Dto>>.OperationSuccess(pagedResult);
