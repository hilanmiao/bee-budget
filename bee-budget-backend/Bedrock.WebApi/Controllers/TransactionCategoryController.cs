...
namespace Bedrock.WebApi.Controllers
{
    ...
    public class TransactionCategoryController : ControllerBase
    {
        ...
        /// 构造函数注入服务层依赖。
        /// </summary>
        /// <param name="transactionCategoryService">交易分类业务逻辑服务。</param>
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] CreateTransactionCategoryDto createDto)
        {
            if (createDto == null)
            {
            ...
            var id = await _transactionCategoryService.CreateAsync(createDto, currentUser.Id);
            var dto = await _transactionCategoryService.GetAsync(id);
            var responseCreate = ApiResponse<TransactionCategoryDto>.OperationSuccess(dto);
        ...
        /// 更新指定的交易分类。
        /// </summary>
        /// <param name="id">交易分类 ID。</param>
        /// <param name="updateDto">更新 DTO，必须包含有效 ID。</param>
        /// <returns>返回被更新的交易分类 ID。</returns>
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(long id, [FromBody] UpdateTransactionCategoryDto updateDto)
        {
            if (updateDto == null)
            {
                var response = ApiResponse<object>.BadRequest("请求数据不能为空。");
            ...
            var updatedId = await _transactionCategoryService.UpdateAsync(updateDto, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteAsync(long id)
        {
            ...
            var deletedId = await _transactionCategoryService.DeleteAsync(id, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteBatchAsync([FromBody] DeleteBatchRequest request)
        {
            ...
            var deletedCount = await _transactionCategoryService.DeleteBatchAsync(request.Ids, currentUser.Id);
        ...
        public async Task<IActionResult> GetAsync(long id)
        {
            ...
            if (dto == null)
            {
                var response = ApiResponse<TransactionCategoryDto>.NotFound();
                return NotFound(response);
            }

            var responseGet = ApiResponse<TransactionCategoryDto>.OperationSuccess(dto);
            return Ok(responseGet);
        }

        /// <summary>
        /// 查询未删除的交易分类记录，支持按名称模糊搜索。
        /// </summary>
            ...
            [FromQuery] string? name = null)
        {
            ...
            var dtos = await _transactionCategoryService.GetAllAsync(name, currentUser.Id);
            var responseAll = ApiResponse<List<TransactionCategoryDto>>.OperationSuccess(dtos);

            return Ok(responseAll);
        }

        /// <summary>
        /// 分页查询未删除的交易分类记录，支持名称模糊搜索。
        /// </summary>
            ...
            [FromQuery] string? orderByType = null)
        {
            ...
            var pagedResult = await _transactionCategoryService.GetPagedAsync(pageNumber, pageSize, name, currentUser.Id, orderByField, orderByType);
            var responsePaged = ApiResponse<PaginationResult<TransactionCategoryDto>>.OperationSuccess(pagedResult);
