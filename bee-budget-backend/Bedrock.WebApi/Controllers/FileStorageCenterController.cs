...
namespace Bedrock.WebApi.Controllers
{
    ...
    [ApiController]
    [Authorize]
    [Route("api/file-storage-center")]
    public class FileStorageCenterController : ControllerBase
    {
        private readonly IFileStorageCenterService _fileStorageCenterService;

        /// <summary>
        /// 构造函数注入服务层依赖。
        /// </summary>
        /// <param name="fileStorageCenterService">文件存储中心业务逻辑服务。</param>
        public FileStorageCenterController(IFileStorageCenterService fileStorageCenterService)
        {
            _fileStorageCenterService = fileStorageCenterService;
        }
        ...
        /// <returns>返回创建成功的文件存储中心信息。</returns>
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] CreateFileStorageCenterDto createDto)
        {
            if (createDto == null)
            {
            ...
            var id = await _fileStorageCenterService.CreateAsync(createDto, currentUser.Id);
            var dto = await _fileStorageCenterService.GetAsync(id);
            var responseCreate = ApiResponse<FileStorageCenterDto>.OperationSuccess(dto);
        ...
        /// 更新指定的文件存储中心。
        /// </summary>
        /// <param name="id">文件存储中心 ID。</param>
        /// <param name="updateDto">更新 DTO，必须包含有效 ID。</param>
        /// <returns>返回被更新的文件存储中心 ID。</returns>
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(long id, [FromBody] UpdateFileStorageCenterDto updateDto)
        {
            if (updateDto == null)
            {
                var response = ApiResponse<object>.BadRequest("请求数据不能为空。");
            ...
            var updatedId = await _fileStorageCenterService.UpdateAsync(updateDto, currentUser.Id);
        ...
        /// <returns>返回被删除的文件存储中心 ID。</returns>
        ...
        public async Task<IActionResult> DeleteAsync(long id)
        {
            ...
            var deletedId = await _fileStorageCenterService.DeleteAsync(id, currentUser.Id);
        ...
        public async Task<IActionResult> DeleteBatchAsync([FromBody] DeleteBatchRequest request)
        {
            ...
            var deletedCount = await _fileStorageCenterService.DeleteBatchAsync(request.Ids, currentUser.Id);
        ...
        public async Task<IActionResult> GetAsync(long id)
        {
            ...
            if (dto == null)
            {
                var response = ApiResponse<FileStorageCenterDto>.NotFound();
                return NotFound(response);
            }

            var responseGet = ApiResponse<FileStorageCenterDto>.OperationSuccess(dto);
        ...
        {
            var dtos = await _fileStorageCenterService.GetAllAsync(name);
            var responseAll = ApiResponse<List<FileStorageCenterDto>>.OperationSuccess(dtos);
        ...
        {
            var pagedResult = await _fileStorageCenterService.GetPagedAsync(pageNumber, pageSize, name, orderByField, orderByType);
            var responsePaged = ApiResponse<PaginationResult<FileStorageCenterDto>>.OperationSuccess(pagedResult);
