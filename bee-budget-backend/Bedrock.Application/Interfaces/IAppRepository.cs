...
namespace Bedrock.Application.Interfaces
{
    ...
    public interface IAppRepository
    {
        ...
        Task<long> CreateAsync(App entity);
        ...
        Task<int> UpdateAsync(App entity);
        ...
        Task<int> DeleteAsync(App entity);
        ...
        Task<List<App>> GetByIdsAsync(IEnumerable<long> ids);

        /// <summary>
        /// 查询未删除的App应用记录，支持按App应用名称模糊搜索和状态精确筛选。
        /// </summary>
        /// <param name="name">App应用名称，用于模糊搜索（可选）。</param>
        /// <returns>匹配条件的未删除记录集合（可能为空）。</returns>
        Task<List<App>> GetAllAsync(string? name = null);

        /// <summary>
        /// 分页查询未删除的App应用记录，支持名称、手机号码模糊搜索和状态精确筛选。
        /// </summary>
        ...
        /// </returns>
        Task<(List<App> Data, int TotalCount)> GetPagedAsync(
            int pageNumber,
            int pageSize,
            string? name = null,
        ...
        Task<App?> GetByAppIdAsync(string name);
