  ...
  <div class="app-container">
            ...
            <div class="table-container__body" ref="tableRef">
              <el-table v-loading="tableLoading"
                ...
                <el-table-column
                    ...
                    width="50"
                    align="center"
                    :selectable="(row) => !row.isPublic"
                />
                <el-table-column label="id" align="right" width="100" fixed="left" key="id" prop="id" sortable="custom"
                                 :sort-orders="['descending', 'ascending']"/>
                <el-table-column label="分类名称" key="name" prop="name"/>
                <el-table-column label="图标" align="center" key="icon" prop="icon">
                  <template #default="scope">
                    <svg-icon :icon-class="scope.row.icon"/>
                  </template>
                </el-table-column>
                <el-table-column label="公共分类" align="center" key="suspend" prop="suspend">
                  <template #default="scope">
                    <el-tag type="success" v-if="!scope.row.isPublic">否</el-tag>
                ...
                <el-table-column label="操作" align="center" width="240" fixed="right">
                  <template #default="scope">
                    <el-button link type="primary" icon="Edit" @click="onUpdate(scope.row)" v-show="!scope.row.isPublic"
                    >编辑
                    </el-button>
                    <el-button link type="primary" icon="Delete" @click="onDelete(scope.row)"
                               v-show="!scope.row.isPublic"
                    >删除
                    </el-button>
                  </template>
                </el-table-column>
    ...
    <el-dialog :title="dialogTitle" v-model="isDialogOpen" width="800px" append-to-body>
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="110">
        <el-form-item label="分类名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入分类名称" maxlength="6" show-word-limit/>
        </el-form-item>
        <el-form-item label="图标" prop="remark">
          <el-popover
          ...
          >
            <template #reference>
              <el-input v-model="formData.icon" placeholder="点击选择图标" @blur="showSelectIcon"
...
<script setup name="TransactionCategory">
// --- 框架工具等相关 ---
...
// --- api 相关 ----
import {
  getTransactionCategoryPaged,
  getTransactionCategory,
  createTransactionCategory,
  updateTransactionCategory,
  deleteTransactionCategory,
  batchDeleteTransactionCategory,
} from "@/api/ledger/transaction-category"
...
async function loadList() {
  ...
  try {
    const response = await getTransactionCategoryPaged(searchParams.value)
...
async function onDelete(row) {
  ...
  try {
    ...
    let response
    if (delIds.length > 1) {
      response = await batchDeleteTransactionCategory(delIds)
    } else {
      response = await deleteTransactionCategory(delIds[0])
    }
...
async function onUpdate(row) {
  reset()
  try {
    const response = await getTransactionCategory(row.id)
    const {success, data, message} = response
    if (success) {
      formData.value = data
    ...
    if (valid) {
      ...
      try {
        const response = await (formData.value.id !== null
            ? updateTransactionCategory(formData.value)
            : createTransactionCategory(formData.value))
        const {success, data, message} = response
        if (success) {
