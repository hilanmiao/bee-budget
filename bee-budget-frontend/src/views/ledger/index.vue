  ...
  <div class="app-container">
          ...
          <div class="table-container">
            ...
            <div class="table-container__body" ref="tableRef">
              <el-table v-loading="tableLoading"
                ...
                <el-table-column label="账本名称" key="name" prop="name"/>
    ...
    <el-dialog :title="dialogTitle" v-model="isDialogOpen" width="800px" append-to-body>
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="110">
        <el-form-item label="账本名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入账本名称" maxlength="8" show-word-limit/>
        </el-form-item>
...
async function loadList() {
  ...
  try {
    const response = await getLedgerPaged(searchParams.value)
...
async function onDelete(row) {
  ...
  try {
    ...
    let response
    if (delIds.length > 1) {
      response = await batchDeleteLedger(delIds)
    } else {
      response = await deleteLedger(delIds[0])
    }
...
async function onUpdate(row) {
  ...
  try {
    const response = await getLedger(ledgerId)
    const {success, data, message} = response
    if (success) {
      formData.value = data
    ...
    if (valid) {
      ...
      try {
        const response = await (formData.value.id !== null
            ? updateLedger(formData.value)
            : createLedger(formData.value))
        const {success, data, message} = response
        if (success) {
