  ...
  <div class="app-container">
      ...
      <div class="layout-center">
        <el-card shadow="always" class="search">
          <el-form :model="searchParams" ref="searchRef" :inline="true" label-width="100px" @submit.prevent="onSearch">
            ...
            <el-form-item label="字典分类编码" prop="code">
              <el-input
            ...
            <el-form-item label="字典分类状态" prop="status">
              <el-select
          ...
          <div class="table-container">
            ...
            <div class="table-container__body" ref="tableRef">
              <el-table v-loading="tableLoading"
                ...
                <el-table-column label="字典分类名称" key="name" prop="name"/>
                <el-table-column label="字典分类编码" align="center">
    ...
    <el-dialog :title="dialogTitle" v-model="isDialogOpen" width="800px" append-to-body>
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="110">
        <el-form-item label="字典分类名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入字典分类名称"/>
        </el-form-item>
        <el-form-item label="字典分类编码" prop="code">
          <el-input v-model="formData.code" placeholder="请输入字典分类编码"/>
        </el-form-item>
        <el-form-item label="字典分类状态" prop="status">
...
<script setup name="DictCategory">
// --- 框架工具等相关 ---
...
// --- api 相关 ----
import {
  getDictCategoryPaged,
  getDictCategory,
  createDictCategory,
  updateDictCategory,
  deleteDictCategory,
  batchDeleteDictCategory,
  changeDictCategoryStatus,
} from "@/api/system/dict-category.js"
...
const formData = ref({})
const formRules = ref({
  name: [{required: true, message: "字典分类名称不能为空", trigger: "blur"}],
  code: [{required: true, message: "字典分类编码不能为空", trigger: "blur"}]
})
...
async function loadList() {
  ...
  try {
    const response = await getDictCategoryPaged(searchParams.value)
...
async function onDelete(row) {
  ...
  try {
    ...
    let response
    if (delIds.length > 1) {
      response = await batchDeleteDictCategory(delIds)
    } else {
      response = await deleteDictCategory(delIds[0])
    }
...
async function onStatusChange(row) {
  ...
  try {
    ...
    const response = await changeDictCategoryStatus(row.id, row.status)
...
async function onUpdate(row) {
  reset()
  try {
    const response = await getDictCategory(row.id)
    const {success, data, message} = response
    if (success) {
      formData.value = data
    ...
    if (valid) {
      ...
      try {
        const response = await (formData.value.id !== null
            ? updateDictCategory(formData.value)
            : createDictCategory(formData.value))
        const {success, data, message} = response
        if (success) {
          ElMessage.success('操作成功')
          isDialogOpen.value = false
