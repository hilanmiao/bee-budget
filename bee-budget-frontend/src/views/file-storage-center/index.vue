  ...
  <div class="app-container">
          ...
          <div class="table-container">
            ...
            <div class="table-container__body" ref="tableRef">
              <el-table v-loading="tableLoading"
                ...
                <el-table-column label="文件名称" key="name" prop="name"/>
                <el-table-column label="路径" align="center" key="storePath" prop="storePath">
                  <template #default="scope">
                    <el-link type="primary" :href="`${uploadPrefix}${scope.row.storePath}`" :underline="false"
                             target="_blank">
                      <span> {{ scope.row.storePath }} </span>
                    </el-link>
                  </template>
                </el-table-column>
    ...
    <el-dialog :title="dialogTitle" v-model="isDialogOpen" width="1200px" append-to-body>
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="110">
        ...
        <el-form-item label="文件" prop="storePath">
          <el-upload
              class="list-uploader"
              ref="uploadStorePathRef"
              accept=""
              :limit="1"
              :headers="uploadStorePath.headers"
              :action="uploadStorePath.url"
              :disabled="uploadStorePath.isUploading"
              :on-progress="onStorePathUploadProgress"
              :on-exceed="onStorePathUploadExceed"
              :on-success="onStorePathUploadSuccess"
              :on-error="onStorePathUploadError"
              :on-remove="onStorePathUploadRemove"
              :file-list="formData.storePath"
          >
            <el-button type="primary">上传文件</el-button>
            <template #tip>
              <div class="el-upload__tip">
                仅支持上传1个并且不能超过500MB的文件
              </div>
            </template>
          </el-upload>
        </el-form-item>
...
<script setup name="FileStorageCenter">
// --- 框架工具等相关 ---
...
// --- api 相关 ----
import {
  getFileStorageCenterPaged,
  getFileStorageCenter,
  createFileStorageCenter,
  updateFileStorageCenter,
  deleteFileStorageCenter,
  batchDeleteFileStorageCenter,
} from "@/api/file-storage-center"
...
const isSubmitDisabled = computed(() => {
  if (uploadStorePath.isUploading || formSubmitting.value) {
    return true
  }
  return false
})
...
const uploadStorePathRef = ref(null)
const uploadStorePath = reactive({
  isUploading: false,
  // 设置上传的请求头部
...
async function loadList() {
  ...
  try {
    const response = await getFileStorageCenterPaged(searchParams.value)
...
async function onDelete(row) {
  ...
  try {
    ...
    let response
    if (delIds.length > 1) {
      response = await batchDeleteFileStorageCenter(delIds)
    } else {
      response = await deleteFileStorageCenter(delIds[0])
    }
...
async function onUpdate(row) {
  ...
  try {
    const response = await getFileStorageCenter(dictCategoryId)
    const {success, data, message} = response
    if (success) {
      formData.value = data
      formData.value.storePath = data.storePath?.length ? data.storePath.split(',').map(o => {
        return {name: o, url: o}
      }) : []
  ...
  formRef.value?.validate(async valid => {
    if (valid) {
      ...
      try {
        ...
        formDataClone.storePath = formData.value.storePath?.length ? formData.value.storePath.map(o => o.url).join(',') : null
        const response = await (formDataClone.id !== null
            ? updateFileStorageCenter(formDataClone)
            : createFileStorageCenter(formDataClone))
        const {success, data, message} = response
        if (success) {
...
/* 文件上传中处理 */
const onStorePathUploadProgress = (evt, uploadFile, uploadFiles) => {
  uploadStorePath.isUploading = true
}

/* 文件覆盖前一个 */
const onStorePathUploadExceed = (files, uploadFiles) => {
  uploadStorePathRef.value.clearFiles()
  const file = files[0]
  file.uid = genFileId()
  uploadStorePathRef.value.handleStart(file)
  uploadStorePathRef.value.submit()
}

/* 文件上传成功处理 */
const onStorePathUploadSuccess = (response, uploadFile, uploadFiles) => {
  uploadStorePath.isUploading = false
  const {code, success, data, message} = response
  if (code === 200 && success) {
    uploadFile.url = data
    formData.value.storePath.push({name: uploadFile.name, url: data})
...
/* 文件上传失败处理 */
const onStorePathUploadError = (error, uploadFile, uploadFiles) => {
  console.log(error, uploadFile, uploadFiles)
  uploadStorePath.isUploading = false
}

/* 文件删除处理 */
const onStorePathUploadRemove = (uploadFile, uploadFiles) => {
  formData.value.storePath = formData.value.storePath.filter(o => o.url !== uploadFile.url)
}
